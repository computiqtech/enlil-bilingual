# Recursive Multi-Step Neural Network (RNN)
...
# Provided time and actual values
...
train_size = int(len(actual_values) * 0.8)
train_data = actual_values[:train_size]
train_labels = actual_values[:train_size]
test_data = actual_values[train_size:]
test_labels = actual_values[train_size:]
...
plt.plot(time[train_size:], test_labels, label='Actual Values')
plt.plot(time[train_size:], predictions, label='Predicted Values')
